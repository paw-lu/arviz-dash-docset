...
def icon(session: Session) -> None:
    ...
    for size, file_name in (("16x16", "icon.png"), ("32x32", "icon@2x.png")):
        ...
        session.run(
            "convert",
            "arviz/doc/source/_static/favicon.ico",
            "-resize",
            size,
            "-background",
            "none",
...
def dash(session: Session) -> None:
    ...
    session.run(
        ...
        "--icon=icon.png",
        "--online-redirect-url=https://python.arviz.org/en/stable/",
        ...
        # build docs end up
        f"{LIBRARY_REPOSITORY}/doc/build",
        *session.posargs,
    )
